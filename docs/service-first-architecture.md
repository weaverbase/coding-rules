...
2. Keep interfaces thin. Each interface should validate input, call the service,
   and map the output.
3. Never put business logic in interface handlers. Avoid `subprocess`, queries,
   and domain workflows in API routes, CLI commands, or UI handlers.
4. Service methods use plain domain types. Do not use `Request`, `Response`, or
   framework-specific types in service signatures.
5. Return structured result types from services. Do not return raw dicts,
   untyped objects, or framework responses.
6. Map errors at the boundary. Services raise domain errors; interfaces translate
   them into HTTP status codes, exit codes, UI messages, or job failure states.
7. Adding a new interface should not require changing the service. If it does,
